Bereinige alle Vorherigen Tasks um diese durch ausfuehrlich detaillierte Ultrathink Langzeittasks zu Entwickeln welche die uebergeordnete Aufgabe haben, eine SECURITY-FORENSIK des bestehenden Projekts durchzufuehren. Pruefe HTTP-Headers, Secret-Management, bekannte Vulnerabilities und DSGVO-Compliance. Erstelle einen Implementierungsplan mit Security-Scan-Protokoll, Risiko-Bewertung und Massnahmen-Katalog.

═══ CHAIN-LINK-HEADER ═══ SEQ-08 von 72 ═══ Pipeline: Discovery ═══
← VORHERIGE: SEQ-07 → DISCOVERY-PHASE-07.md
→ NAECHSTE: SEQ-09 → DISCOVERY-PHASE-09.md
...
═══════════════════════════════════════════════════════════════════════════════
DISCOVERY-PHASE-08: SECURITY-FORENSIK UND COMPLIANCE-STATUS
═══════════════════════════════════════════════════════════════════════════════

DEFINITION: Phase 8 fuehrt ein vollstaendiges Security-Audit des bestehenden
Projekts durch. HTTP-Headers, Secret-Exposure, Dependency-Vulnerabilities,
DSGVO-Compliance, Input-Validierung, CSRF-Schutz. Jede Schwachstelle wird
dokumentiert und priorisiert — als Vorbereitung fuer den Director of Engineering.
ERGEBNIS: .ai-discovery-context.md Sektion SECURITY-STATUS vollstaendig befuellt.

ULTRATHINK-LANGZEITTASK 8.1: HTTP-SECURITY-HEADERS ANALYSE
─────────────────────────────────────────────────────────────────────────────
Pruefe die Build-Config auf Security Headers:
  Content-Security-Policy: Vorhanden? Restriktiv genug?
  X-Frame-Options: DENY oder SAMEORIGIN?
  Strict-Transport-Security: max-age Wert?
  X-Content-Type-Options: nosniff?
  Referrer-Policy: Wert?
  Permissions-Policy: Welche Features eingeschraenkt?
Dokumentiere:
  [HEADER] | [STATUS: VORHANDEN/FEHLEND] | [WERT] | [BEWERTUNG: SICHER/UNSICHER]

ULTRATHINK-LANGZEITTASK 8.2: SECRET-EXPOSURE-SCAN
─────────────────────────────────────────────────────────────────────────────
Scanne den gesamten Code nach exponierten Secrets:
  API-Keys, Tokens, Passwoerter in Quellcode-Dateien.
  NEXT_PUBLIC_ Variablen die geheime Daten enthalten.
  Hardcodierte URLs mit Credentials.
  .env Dateien die NICHT in .gitignore stehen.
  Build-Output der Secrets exponiert.
  console.log Statements die sensible Daten ausgeben.

ULTRATHINK-LANGZEITTASK 8.3: DEPENDENCY-VULNERABILITY-AUDIT
─────────────────────────────────────────────────────────────────────────────
Fuehre npm audit durch und dokumentiere:
  Anzahl Critical, High, Medium, Low Vulnerabilities.
  Fuer JEDE Critical/High Vulnerability:
    Paket-Name, Version, CVE-Nummer, Beschreibung.
    Fix verfuegbar? (npm audit fix moeglich?)
    Transitive Dependency? (Nicht direkt, sondern via anderes Paket?)

ULTRATHINK-LANGZEITTASK 8.4: DSGVO-UND-DATENSCHUTZ-COMPLIANCE
─────────────────────────────────────────────────────────────────────────────
Pruefe die Datenschutz-Konformitaet:
  Cookie-Consent-Banner: Vorhanden? GDPR-konform?
  Formulare: Datenschutz-Checkbox vorhanden? Nicht vorselektiert?
  Datenschutzerklaerung: Verlinkt? Erreichbar? Aktuell?
  Impressum: Vorhanden? (Pflicht fuer deutsche Projekte)
  Analytics: Welches Tool? Cookie-basiert oder cookieless?
  Third-Party-Scripts: Welche laden personenbezogene Daten?

ULTRATHINK-LANGZEITTASK 8.5: INPUT-VALIDIERUNG UND INJECTION-SCHUTZ
─────────────────────────────────────────────────────────────────────────────
Pruefe JEDES Formular und jeden User-Input:
  Client-seitige Validierung: Vorhanden? Welche Methode?
  Server-seitige Validierung: Vorhanden? Zod/Yup/manuell?
  dangerouslySetInnerHTML: Wo verwendet? Sanitized?
  SQL Injection Risiko: Falls Datenbank-Zugriff vorhanden.
  XSS Risiko: Unsanitized User-Input im DOM?

═══════════════════════════════════════════════════════════════════════════════
═══ CHAIN-EXIT-FOOTER ═══ SEQ-08 ABGESCHLOSSEN ═══ Pipeline: Discovery ═══
═══════════════════════════════════════════════════════════════════════════════
...
3. CONTEXT-UPDATE:
   [ ] .discovery-state.md aktualisiert (Phase 08 COMPLETE)
   [ ] Erkenntnisse fuer nachfolgende Phasen vorbereitet
   [ ] Naechste Phase wird automatisch getriggert (AUTONOME VERKETTUNG)

═══ DISCOVERY-PHASE-08 ABGESCHLOSSEN → WEITER MIT DISCOVERY-PHASE-09.md ═══
